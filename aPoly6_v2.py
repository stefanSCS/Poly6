...
----functions 'cvxCheck' and  'plotPoly6' were reimplemented to call via 'ctypes' the corresponding C-functions.
'''
...
import ctypes
cfuncs=ctypes.CDLL('cFuncs.so')## 'cFuncs.so' is the shared object compiled from 'cFuncs.c'
cDouble=ctypes.c_double
cInt=ctypes.c_int
cShrt=ctypes.c_short
ptDouble=ctypes.POINTER(cDouble)  ## pointer to double 
ptShrt=ctypes.POINTER(cShrt)
cfuncs.plotDataPoints.restype=cInt
cfuncs.plotDataPoints.argtypes=[ptDouble,ptDouble,cInt,cInt,ptDouble,ptDouble]
cfuncs.convxCheck.restype=None
cfuncs.convxCheck.argtypes=[ptDouble,cInt,cInt,ptShrt,ptShrt,ptDouble,ptDouble,ptDouble]
...
def cvxCheck():
    ...
    if(4*vCoeff[13]*vCoeff[15]-vCoeff[14]*vCoeff[14]<0): ##the case PHI=0
        print('not convex at the top')
        return False
    nTheta=200; nPhi=100
    cvx=cShrt(-1); funcN=cShrt(-1); funcV=cDouble(0); phiV=cDouble(0); thetaV=cDouble(0)
    cfuncs.convxCheck(vCoeff.ctypes.data_as(ptDouble),nPhi,nTheta,ctypes.byref(cvx),ctypes.byref(funcN),ctypes.byref(funcV),ctypes.byref(phiV),ctypes.byref(thetaV))
    funcN=funcN.value
    deg=180/PI
    if(cvx.value):
        return True
        ...
        if(funcN==0):
            print('P6 = {ag1:.7f} <0 at PHI = {ag2:.2f} and THETA = {ag3:.2f}'.format(ag1=funcV.value,ag2=deg*phiV.value,ag3=deg*thetaV.value))
        if(funcN==1):
            print('det1 = {ag1:.7f} <0 at PHI = {ag2:.2f} and THETA = {ag3:.2f}'.format(ag1=funcV.value,ag2=deg*phiV.value,ag3=deg*thetaV.value))
        if(funcN==2):
            print('det2 = {ag1:.7f} <0 at PHI = {ag2:.2f} and THETA = {ag3:.2f}'.format(ag1=funcV.value,ag2=deg*phiV.value,ag3=deg*thetaV.value))
        if(funcN==3):
            print('det3 = {ag1:.7f} <0 at PHI = {ag2:.2f} and THETA = {ag3:.2f}'.format(ag1=funcV.value,ag2=deg*phiV.value,ag3=deg*thetaV.value))  
        return False            
...
def plotPoly6(missingData,addedData,strOption):
    ...
    maxSXY=1.0/vCoeff[16]**(1/6); nSXY=20 
    vvSXY=np.linspace(0.0,maxSXY-0.125*maxSXY/(nSXY-1),nSXY)
    nTheta=100; nThetaPP=nTheta+1
    vvsx=np.zeros(nSXY*(nThetaPP),dtype=cDouble).reshape(nSXY,nThetaPP)
    vvsy=np.zeros(nSXY*(nThetaPP),dtype=cDouble).reshape(nSXY,nThetaPP)
    err=cfuncs.plotDataPoints(vCoeff.ctypes.data_as(ptDouble),vvSXY.ctypes.data_as(ptDouble),nSXY,nTheta,vvsx.ctypes.data_as(ptDouble),vvsy.ctypes.data_as(ptDouble))
    if(err):
        print('error in cFuncs: malloc did not succeed (free memory and try again)')
        exit()
    for kk in range(0,nSXY):
        ax_1.plot(vvsx[kk,:],vvsy[kk,:],color='k'); ax_1.plot(-vvsx[kk,:],-vvsy[kk,:],color='k')
        print('Plotting level curve {level}/{nLevel}: sxy = {shear} ...Done'.format(level=str(kk+1),nLevel=str(nSXY),shear="{:.3f}".format(vvSXY[kk])))   
    ax_1.grid()
